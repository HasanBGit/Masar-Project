...
description: >
  ...
  alerts, digital contract signing, warranty/retention tracking, ZATCA
  e-invoicing, or the contract legal-agent chatbot (RAG). Triggers on:
  "contract terms", "payment milestone", "ZATCA", "Fatoora", "e-invoice",
  "contract vs actual", "retention period", "escrow", "legal agent",
  "contract chatbot". Prevents gating payment on contractor self-assertion
  instead of trust-evidence's verified evidence - payment verification has
  "no teeth" without that link, per the cross-module note below.
...
**Owns:** `Contract` (structured payment schedule, milestone definitions, scope baseline), `PaymentMilestone`, the contract-vs-actual tracker, ceiling/threshold alert logic, the plain-Arabic contract summary layer, warranty/retention period fields, ZATCA e-invoice generation, and the contract legal-agent chatbot (`LegalAgentDocument`, `services.ingest_contract_for_legal_agent`, `services.ask_legal_agent`).

**Legal-agent chatbot (RAG) - implemented in `contract_payments/services.py`:** answers contract/ZATCA questions grounded only in (a) this project's own structured contract data and (b) the static `references/zatca-fatoora-compliance.md` file already checked into this repo. No live web-fetching of external documentation happens in this implementation. Embeddings (Voyage AI, `VOYAGE_API_KEY`) and answer generation (Anthropic, `ANTHROPIC_API_KEY`) are both optional and fail closed with `LegalAgentNotConfigured` (surfaced as HTTP 503) when unset - the rest of the module works with no key configured at all. **If a future change adds live fetching of an external document (e.g. ZATCA's own published spec) - any outbound HTTP request must use a neutral/generic User-Agent and must never include a personal email address, "Truepoint"/"San3"/"Masar", or any other project-identifying string in headers, query params, or referrers.**

`LegalAgentDocument.embedding` is a real **pgvector** column (`VectorField`, HNSW cosine index) - not an in-app vector store, so it deploys on the same Supabase Postgres instance as everything else with no separate vector-DB service. Local dev requires the `vector` Postgres extension to actually be installed on the running Postgres server (Supabase ships it pre-enabled; Homebrew Postgres does not - see the note this SKILL.md's own build history left in `contract_payments/models.py`'s `LEGAL_AGENT_EMBEDDING_DIMENSIONS` comment).
