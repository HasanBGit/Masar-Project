...
@pytest.mark.django_db
def test_record_post_requires_owner_role(project, project_manager_user):
    response = _client(project_manager_user).post(
        f"{BASE}/record/", {"project": project.id, "practical_completion_date": "2026-01-01"}
    )
...
@pytest.mark.django_db
def test_om_verify_rejected_for_project_manager(project, om_item, project_manager_user):
    response = _client(project_manager_user).post(f"{BASE}/om-checklist/{om_item.id}/verify/")
    assert response.status_code == 403
...
@pytest.mark.django_db
def test_defect_resolve_twice_second_is_400(project, owner_user, project_manager_user):
    defect = report_defect(project=project, reported_by=owner_user, unit_or_zone="Unit 4B", title="Leak")
    client = _client(owner_user)

    _client(project_manager_user).post(f"{BASE}/post-handover-defects/{defect.id}/acknowledge/")
...
@pytest.mark.django_db
def test_defect_resolve_rejected_for_project_manager(project, owner_user, project_manager_user):
    defect = report_defect(project=project, reported_by=owner_user, unit_or_zone="Unit 4B", title="Leak")
    response = _client(project_manager_user).post(f"{BASE}/post-handover-defects/{defect.id}/resolve/")
    assert response.status_code == 403
...
@pytest.mark.django_db
def test_defect_resolve_only_logs_one_audit_event(project, owner_user, project_manager_user):
