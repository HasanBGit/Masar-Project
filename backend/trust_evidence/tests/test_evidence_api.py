...
def test_evidence_on_decision_does_not_satisfy_payment_milestone_gate(project, owner_user, contractor_user):
    ...
    record = submit_evidence(
        ...
        submitted_by=contractor_user, caption="Wrong subject type", captured_at=timezone.now(),
    )
...
@pytest.mark.django_db
def test_verification_is_project_scoped(project, owner_user, contractor_user):
    ...
    record = submit_evidence(
        ...
        submitted_by=contractor_user, caption="Project A evidence", captured_at=timezone.now(),
    )
    verify_evidence(record, owner_user)
...
def test_evidence_rejects_out_of_range_coordinates(project, contractor_user, field, value):
    payload = {
        "project": project.id,
    ...
    response = _client(contractor_user).post(f"{BASE}/evidence/", payload)
    assert response.status_code == 400
    assert field in response.data


@pytest.mark.django_db
def test_evidence_rejects_future_captured_at(project, contractor_user):
    payload = {
        "project": project.id,
        "subject_type": "milestone",
        "subject_id": "1",
...
@pytest.mark.django_db
def test_dispute_export_restricted_to_owner_admin(project, owner_user, consultant_user, contractor_user):
    assert _client(contractor_user).get(f"{BASE}/dispute-export/", {"project": project.id}).status_code == 403
...
@pytest.mark.django_db
def test_evidence_create_with_unknown_project_is_client_error_not_500(contractor_user):
    payload = {
        "project": 999999,
        "subject_type": "milestone",
        "subject_id": "1",
