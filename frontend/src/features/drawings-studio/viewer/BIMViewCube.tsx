...
// Truepoint has no i18n infrastructure yet (Module 8 is Phase 3) - this
// stands in for react-i18next's `t()`, always returning the English
// default OpenConstructionERP ships as a fallback.
function t(_key: string, opts: { defaultValue: string }): string {
...
export function BIMViewCube({
  ...
  /**
   * Incrementing key: forces React to unmount the old <canvas> and mount a
   * brand-new one on every cleanup cycle. This prevents the poisoned-context
   * bug where forceContextLoss() on the old canvas made the next
   * THREE.WebGLRenderer() throw even though WebGL is available.
   */
  ...
  useEffect(() => {
    ...
    return () => {
      ...
      // Do NOT call forceContextLoss() here: it permanently poisons this
      // canvas element. React StrictMode double-invokes effects on the same
      // DOM node, so the second mount would find the context already dead and
      // show "no 3D context" even though WebGL is available. renderer.dispose()
      // correctly frees all GPU resources; the native GL context is released
      // by the browser when the canvas leaves the DOM.
      renderer.dispose();
      // Give the next mount a fresh canvas element so there is never any
      // lingering context state to inherit.
      setCubeKey((k) => k + 1);
    };
  }, [size]);
    ...
    >
      {glUnavailable ? (
        ...
        <div
          ...
          className="w-full h-full flex flex-col items-center justify-center gap-0.5 rounded-lg border border-dashed border-sand bg-white/80 text-navy/40 px-1.5 text-center"
          style={{ width: size, height: size }}
        >
