...
// Inlined from OpenConstructionERP's useBIMViewerStore - Truepoint has no
// equivalent store, and this is a plain string union, not app state.
...
export class SceneManager {
  ...
  constructor(canvas: HTMLCanvasElement) {
    ...
    this.gridHelper = new THREE.GridHelper(20, 20, 0x94a3b8, 0xe2e8f0);
    this.gridHelper.visible = true; // visible by default for clear 3D floor perspective
    this.scene.add(this.gridHelper);

    // Resize observer - also request a render so the new viewport is drawn.
    this.resizeObserver = new ResizeObserver(() => {
      const hadPendingFit = this._pendingInitialFit && this.container.clientWidth > 10;
      this.updateSize();
      if (hadPendingFit) {
        this._pendingInitialFit = false;
        const pendingBox = this._pendingZoomToFitBbox;
        this._pendingZoomToFitBbox = null;
        this.zoomToFit(pendingBox || undefined);
      }
      this._needsRender = true;
    });
  ...
  private _pendingInitialFit = false;
  private _pendingZoomToFitBbox: THREE.Box3 | null = null;
  ...
  zoomToFit(bbox?: THREE.Box3): void {
    if (this.container.clientWidth <= 10) {
      this._pendingInitialFit = true;
      this._pendingZoomToFitBbox = bbox || null;
      return;
    }
    ...
    if (bbox) {
      ...
      this.scene.traverse((obj) => {
        // Skip helpers, grids, lights, cameras, or invisible hover markers
        if (
          !obj.visible ||
    ...
    const dist = (maxDim / (2 * Math.tan(effectiveFOV / 2))) * 1.25;
  ...
  dispose(): void {
    ...
    // renderer.dispose() frees all GPU-side resources (programs, buffers,
    // textures, render targets). We intentionally do NOT call
    // forceContextLoss() here: that permanently poisons the canvas element,
    // which breaks React StrictMode (where effects run twice on the same DOM
    // node - mount → cleanup → remount). On the second mount, Three.js finds
    // the context already killed and throws "Error creating WebGL context",
    // even though the browser supports WebGL2. The canvas's native GL context
    // is released naturally when React unmounts the element from the DOM, so
    // the browser's context cap (~8-16) is respected without forceContextLoss.
