...
export class DeferredTeardown {
  ...
  private readonly scheduleFn: TeardownScheduler;
  private readonly cancelFn: TeardownCanceller;

  constructor(
    scheduleFn: TeardownScheduler = (run) =>
      setTimeout(run, 0) as unknown as number,
    cancelFn: TeardownCanceller = (h) => clearTimeout(h),
  ) {
