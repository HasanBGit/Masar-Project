...
export function AuthOverlay({ onLogin, error, darkMode }: Props) {
  ...
  return (
    <div className={`absolute inset-0 z-10 flex flex-col items-center justify-center p-6 backdrop-blur-sm ${darkMode ? "bg-[#0a1628]/80" : "bg-white/80"}`}>
      <div className={`w-full max-w-md rounded-3xl border p-8 text-center shadow-2xl ${darkMode ? "border-white/10 bg-[#111c2d]" : "border-gray-200 bg-white"}`}>
        <div className="mx-auto mb-6 flex h-16 w-16 items-center justify-center rounded-2xl bg-[#EA4335]/15 text-[2rem] text-[#EA4335]">
          ✉
        </div>
        
        <h2 className={`font-display text-[1.4rem] font-bold ${darkMode ? "text-white" : "text-gray-900"}`}>
        ...
        {error && (
          <div className="mt-4 rounded-xl bg-red-500/10 p-3 text-[0.8rem] text-red-500 border border-red-500/20">
            {error}
          </div>
        )}
